...
console.log(data.results[i].title,data.results[i].vote_average,data.results[i].genres)

}
let genre=prompt('The loai ban thich: ');

//console.log(data.results[1].genres)
for(let i=0;i<data.results.length;i++)
{  
     for(let j=0;j<data.results[i].genres.length;j++)
    {
        if(data.results[i].genres[j]==genre)
        {
...
let rate=prompt('Nhap diem toi thieu: ');
for(let i=0;i<data.results.length;i++)
{
    if(rate<=data.results[i].vote_average){
        console.log(data.results[i].title)
    }
}
